...
class AmbulanceVehicleEntry:
    display_name: str
    model: str
    part_config: str


class AmbulanceVehicleExtractor:
    ...
    def extract(self, available) -> list[AmbulanceVehicleEntry]:
        entries: list[AmbulanceVehicleEntry] = []
        seen = set()
    ...
    def format_entries(self, entries: Iterable[AmbulanceVehicleEntry]) -> str:
        lines = ["AMBULANCE = ["]
...
def main() -> None:
    ...
    output_path = Path(__file__).resolve().parent / "ambulance_vehicles.txt"
    ...
    extractor = AmbulanceVehicleExtractor()
